...
void MFRC522_Init(void) {
	...
	MFRC522_Wr(TPRESCALERREG, 0xA9);
//TPrescaler_Lo = 0xA9; f(Timer) = 13.56
//Mhz / (2*Tprescaler + 1) = 40 Khz
	MFRC522_Wr(TRELOADREGH, 0x03);
	MFRC522_Wr(TRELOADREGL, 0xE8); //Reload Timer con 0x3E8 = 1000, son 25ms
//antes que el timeout
...
/*-------------------------------------------------------------------*/
unsigned char MFRC522_CALC_CRC(unsigned char *data, unsigned char length,
...
/*-------------------------------------------------------------------*/
unsigned char MFRC522_Request(unsigned char *bufferATQA,
	...
	validBits = 7;
	status = MFRC522_TrasceiveData(&command, 1, bufferATQA, bufferSize,
			&validBits, 0x0, false);
	if (status != STATUS_OK)
		return status;
...
/*-------------------------------------------------------------------*/
unsigned char MFRC522_TrasceiveData(unsigned char *sendData,
		unsigned char sendLen, unsigned char *backData, unsigned char *backLen,
		unsigned char *validBits, unsigned char rxAlign, bool checkCRC) {
	unsigned char waitIrq = 0x30;
	return MFRC522_CommuncicateWithPICC(PCD_TRANSCEIVE, waitIrq, sendData,
			sendLen, backData, backLen, validBits, rxAlign, checkCRC);
}
/*-------------------------------------------------------------------*/
unsigned char MFRC522_CommuncicateWithPICC(unsigned char command,
		unsigned char waitIrq, unsigned char *sendData, unsigned char sendLen,
	...
	MFRC522_Wr_Buff(FIFODATAREG, sendLen, sendData); //Escribo sendData en
//FIFO
	...
	if (i != 0) {
		...
		if (backData && backLen) {
			n = MFRC522_Rd(FIFOLEVELREG); //Leo cantidad bits
										// almacenados en FIFO
			if (n > *backLen)
				return STATUS_NO_ROOM;
...
//Copy the known bits from uid->uidByte [] to buffer []
		...
		while (!selectDone) {
			if (currentLevelKnownBits >= 32) {
				buffer[1] = 0x70; //Calculo BCC - Block Check Character
...
//Calculo CRC_A
			...
			result = MFRC522_TrasceiveData(buffer, bufferUsed, responseBuffer,
...
unsigned char MFRC522_HaltA(void) {
	...
	if (result == STATUS_OK)
		return result;
	result = MFRC522_TrasceiveData(buffer, sizeof(buffer), NULL, 0, NULL, 0,
			false);
...
//2. Limpiar el buffer interno
	MFRC522_Wr(FIFOLEVELREG, 0x80);
//Limpio el buffer enviandole un "1" en el bit 7
	MFRC522_Wr_Buff(FIFODATAREG, 25, bytes);
//Cargo todo el buffer con "0"
